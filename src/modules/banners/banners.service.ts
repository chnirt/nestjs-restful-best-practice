...
export class BannersService {
  ...
  async insert(createBannerDto: CreateBannerDto): Promise<boolean> {
    ...
    const foundBanner = await getMongoRepository(BannerEntity).findOne({ position, published: true })
  ...
  async findOneAndReplace(
    ...
    let foundBanner = await getMongoRepository(BannerEntity).findOne({ position, published: true })
